...
export default function Amenity() {
    ...
    useEffect(() => {
      ...
      const getAllAmenities = async () => {
        const { data } = await axios.get(`http://localhost:3000/api/v1/amenities?limit=${limit}&page=${pageNum+1}`);
  ...
  return (
    ...
    <Button name="Add Branch " icon={CiSquarePlus}  navigate = "/anemities/addAnemity"/>
        <div className='grid  grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mx-8'>
                {
                    amenities.map((amenity)=>(
                        ...
                        <div className='flex justify-center  h-52  items-center  flex-col border border-3 rounded-2xl border-main-800 bg-grey-500'>
                             ...
                             <img src={amenity?.images[0]} alt="Amenity" className=" w-16 h-16 object-cover" />
                             <p className="text-main-800  text-lg font-bold">{amenity.name}</p>
                             </div>
                             <Link to={`/anemities/editAnemity/${amenity.id}`}
